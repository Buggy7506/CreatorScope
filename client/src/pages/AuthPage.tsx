...
const AppleIcon = () => <img src="/assets/apple.svg" alt="" className="auth-social-svg" />;
...
const AuthPage = ({ user, onLogin }: AuthPageProps) => {
  ...
  const getErrorMessage = (caughtError: unknown) => {
    if (axios.isAxiosError(caughtError)) {
      return caughtError.response?.data?.message || caughtError.message || 'Something went wrong. Please try again.';
    }
  ...
  const handleSocialLogin = (provider: 'google' | 'apple' | 'microsoft') => {
    window.location.href = `${apiOrigin}/api/v1/connect/${provider}`;
  };
    ...
    <section className="auth-shell">
      ...
      <div className="auth-grid">
        <div className="auth-copy">
          <div className="auth-copy-badge">VunaFlow-inspired creator intelligence</div>
          <h1 className="auth-copy-title">
            {isRegister ? 'Create your revenue workspace' : 'Sign in to your creator command center'}
          </h1>
          <p className="auth-copy-text">
            Manage analytics, campaigns, billing, and social channels through a calm green dashboard styled like a modern automation platform.
          </p>

          <div className="auth-copy-list">
            <div className="auth-copy-item">Unified TikTok, YouTube & Instagram analytics</div>
            <div className="auth-copy-item">AI captions, reports & viral timing signals</div>
            <div className="auth-copy-item">Secure email, Google, Microsoft & Apple sign-in</div>
          </div>
        </div>

        <div className="auth-card">
          <div className="auth-card-header">
            <p className="auth-card-kicker">{isRegister ? 'Start free' : 'Welcome back'}</p>
            <h2>{isRegister ? 'Create account' : 'Log in'}</h2>
            <p>{isRegister ? 'Your trial dashboard is ready after one step.' : 'Continue with your account or a social provider.'}</p>
          </div>
          ...
          <div className="auth-social-grid">
            ...
            <button type="button" onClick={() => handleSocialLogin('microsoft')} className="auth-social-button">
              <MicrosoftIcon /> Microsoft
            </button>
            <button type="button" onClick={() => handleSocialLogin('apple')} className="auth-social-button">
              <AppleIcon /> Apple
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="auth-step-block">
            {isRegister ? (
              <div className="auth-field-group">
                ...
                <input id="name" type="text" value={name} onChange={(event) => setName(event.target.value)} placeholder="Ada Creator" className="auth-input" />
              </div>
            ) : null}

            <div className="auth-field-group">
              <label className="auth-field-label" htmlFor="email">Email</label>
              <input id="email" type="email" value={email} onChange={(event) => setEmail(event.target.value)} placeholder="you@example.com" className="auth-input" />
            </div>

            <div className="auth-field-group">
              <label className="auth-field-label" htmlFor="password">Password</label>
              <input id="password" type="password" value={password} onChange={(event) => setPassword(event.target.value)} placeholder="Enter your password" className="auth-input" />
            </div>

            {isRegister ? (
              <div className="auth-field-group">
                <label className="auth-field-label" htmlFor="confirmPassword">Confirm password</label>
                <input id="confirmPassword" type="password" value={confirmPassword} onChange={(event) => setConfirmPassword(event.target.value)} placeholder="Confirm your password" className="auth-input" />
              </div>
            ) : null}
